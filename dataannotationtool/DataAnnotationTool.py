...
def get_class_info():
    ...
    """
    classInfoPath = os.path.join(os.getcwd(), 'classifier', 'class_info.json')
...
def data_annotation_tool(traningDataDir):
    """
    ...
    try:
        ...
        colors = [[255, 0, 0], [0,255,0], [0,0,255], [1,1,1]]
        colorIdx = 0
    ...
    for imgFn in os.listdir(traningDataDir):
        print(imgFn)
        im_RGB = mpimg.imread(os.path.join(traningDataDir, imgFn))
        im_viz = im_RGB.copy()
        ...
        # otherwise label this image
            ...
            while True:
                ...
                mask = fig_annotate(fig, ax, implot, im_viz, im_RGB, color)
...
def fig_annotate(fig, ax, implot, im_viz, img_RGB, color=[1, 0, 1]):
    ...
    # mask = np.zeros_like(ax.get_images()[0].get_array()[:,:,0], dtype=np.uint8) # Initialize mask with all zeros
    print(mask.shape)
    while True:
        ...
        if tool_selection.get('pressed') == '1':
            ...
            newMask = lasso_tool(fig, ax, implot, im_viz, color)
...
class LassoPixels(object):
    def __init__(self, fig, ax, implot, im_viz, color=[1, 0, 1]):
        self.fig = fig
        self.ax = ax
        self.im = implot
        self.im_viz = im_viz
...
def lasso_tool(fig, ax, implot, im_viz, color=[1, 1, 1]):

    ax.set_title('Lasso tool')
    fig.canvas.draw_idle()
    print('lasso tool')
    im_viz_old = im_viz.copy()
    lasso = LassoPixels(fig, ax, implot, im_viz, color=color)
